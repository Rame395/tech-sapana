...
export default function Services() {
  const [activeCap, setActiveCap] = useState<CapabilityKey>("dashboards");
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isSubmitted, setIsSubmitted] = useState(false);

  const activeData = capabilities[activeCap];
              ...
              {(Object.keys(capabilities) as CapabilityKey[]).map((key) => (
                ...
                >
                  ...
                  <span className={activeCap === key ? "text-blue-600 dark:text-blue-400" : "text-gray-400 dark:text-gray-500"}>
                    {capabilities[key].icon}
                  </span>
                  {capabilities[key].title.split(" ")[0]} {capabilities[key].title.split(" ")[1]}
                </button>
              ))}
            </div>
